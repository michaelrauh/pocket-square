...
(define (single-fold r)
  (displayln (string-append "folding initial text: " (result-filename r)))
...
(define (double-fold res1 res2)
  (displayln
   (string-append "merging: " (result-filename res1) " with: " (result-filename res2)))
  ...
  (displayln (string-append "adding this many squares: "
                            ...
                            " ("
                            (number->string (* 100
                                               (/ (length (get-net-new old-registry current-squares))
                                                  (exact->inexact (length current-squares)))))
                            "%) "
                            "to a registry with: "
                            (number->string (length old-squares))
                            " squares preexisting"))
